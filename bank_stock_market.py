...
sina_default = "http://vip.stock.finance.sina.com.cn/corp/go.php/vMS_MarketHistory/stockid/STOCK_CODE.phtml"
sina_specific = "http://vip.stock.finance.sina.com.cn/corp/go.php/vMS_MarketHistory/stockid/STOCK_CODE.phtml?year=YEAR&jidu=JIDU"
            ...
            if len(stock_data_raw) == 2:
                ...
                for i in range(2,len(stock_data_raw)):
                    ...
                    stock_data_day = {"date": stock_data_day[0], "open": float(stock_data_day[1]), 
                                      ...
                                      "amount": float(stock_data_day[6])}
                    stock_data_jidu.append(stock_data_day)
                stock_data_year.extend(stock_data_jidu)
        stock_data.extend(stock_data_year)
    stock_data = pandas.DataFrame(stock_data, columns=["date", "open", "high", "low", "close", "volume", "amount"])
